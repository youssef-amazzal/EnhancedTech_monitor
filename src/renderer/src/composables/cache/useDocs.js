...
export const useDocs = () => {
  ...
  const { state, isReady } = useAsyncState(supabase.rpc('get_documents_json', {part: task.part.id}), {data: []})
  ...
  whenever(() => isReady.value && countDirtyPages.value === 0, () => {
  ...
  const saveChanges = async () => {
    ...
    const {state : state_2, isReady : isReady_2} = useAsyncState(
      supabase
        .from('pages')
        .delete()
        .in('id', deletedPages.value.map(p => p.id)),
      {data: []}
    )
  ...
  return {remoteDocuments, localDocuments, localPages, countCompleted, countDirtyPages, saveChanges}
}
